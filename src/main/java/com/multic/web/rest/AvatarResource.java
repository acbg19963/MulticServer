...
public class AvatarResource {
    ...
    @GetMapping("/avatars")
    @Timed
    public ResponseEntity<List<Avatar>> getAllAvatars(@ApiParam Pageable pageable, @RequestParam(required = false) String filter) {
        if ("estudiante-is-null".equals(filter)) {
            log.debug("REST request to get all Avatars where estudiante is null");
            return new ResponseEntity<>(StreamSupport
                .stream(avatarRepository.findAll().spliterator(), false)
                .filter(avatar -> avatar.getEstudiante() == null)
